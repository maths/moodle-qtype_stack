...
$notcompiled = $DB->get_recordset_sql('SELECT count(*) as notcompiled FROM {question} q, {qtype_stack_options} o WHERE q.id = o.questionid AND q.hidden = 0 AND o.compiledcache = ?;', ['{}']);
...
$compiled = $DB->get_recordset_sql('SELECT count(*) as compiled FROM {question} q, {qtype_stack_options} o WHERE q.id = o.questionid AND q.hidden = 0 AND NOT o.compiledcache = ?;', ['{}']);
...
echo '<p>This tool only acts on succesfully compiled questions, to compile questions run the bulk tester or preview/use those questions. Currently there are ' . $ncompiled . ' compiled questions and ' . $nnotcompiled . ' questions that have not been succesfully compiled.';
...
if (data_submitted() && optional_param('includes', false, PARAM_BOOL)) {
	 ...
	 */ 
	$qs = $DB->get_recordset_sql('SELECT q.id as questionid FROM {question} q, {qtype_stack_options} o WHERE q.id = o.questionid AND q.hidden = 0 AND ' . $DB->sql_like('o.compiledcache', ':trg') . ';', ['trg' => '%"includes"%']);
...
if (data_submitted() && optional_param('jsxgraphs', false, PARAM_BOOL)) {
	 ...
	 */ 
	$qs = $DB->get_recordset_sql('SELECT q.id as questionid FROM {question} q, {qtype_stack_options} o WHERE q.id = o.questionid AND q.hidden = 0 AND ' . $DB->sql_like('o.compiledcache', ':trg', false) . ';', ['trg' => '%jsxgraph%']);
	echo '<h4>Questions containing JSXGraph related terms</h4>';
	echo '<table><thead><tr><th>Question</th><th>[[jsxgraph]]</th><th>&lt;jsxgraph</th><th>Other</th></tr></thead><tbody>';
	// Load the whole question, simpler to get the contexts correct that way.
	foreach ($qs as $item) {
		$q = question_bank::load_question($item->questionid);
		$block = 'false';
...
if (data_submitted() && optional_param('script', false, PARAM_BOOL)) {
	 ...
	 */ 
	$qs = $DB->get_recordset_sql('SELECT q.id as questionid FROM {question} q, {qtype_stack_options} o WHERE q.id = o.questionid AND q.hidden = 0 AND ' . $DB->sql_like('o.compiledcache', ':trg', false) . ';', ['trg' => '%<script%']);
