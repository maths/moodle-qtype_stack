 ...
 */
define("qtype_stack/library",["core/ajax","core_filters/events"],(function(Ajax,CustomEvents){var contextId=null,libraryDiv=null;function libraryRender(e){const filepath=e.target.getAttribute("data-filepath");libraryDiv.classList.add("loading"),Ajax.call([{methodname:"qtype_stack_library_render",args:{context:contextId,filepath:filepath},done:function(response){libraryDiv.classList.remove("loading"),function(response){libraryDiv.innerHTML=response.questionrender,CustomEvents.notifyFilterContentUpdated(libraryDiv)}(response)},fail:function(response){libraryDiv.classList.remove("loading"),function(response){libraryDiv.innerHTML="Something went wrong. "+JSON.stringify(response)}(response)}}])}return{setup:function(cId,lDiv){contextId=cId,libraryDiv=document.getElementById(lDiv),document.querySelectorAll(".library-file-link").forEach((function(elem){elem.addEventListener("click",libraryRender)}))}}}));
