...
update_state(newUsed,newAvailable){var newState={used:newUsed.map((usedList)=>usedList.map((used)=>used.toArray())),available:newAvailable.toArray()};this.state=newState;if(JSON.stringify(newState)!==JSON.stringify(this.history[0])){this.history.unshift(newState);}
