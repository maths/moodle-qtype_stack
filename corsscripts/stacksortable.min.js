...
generate_used(){for(const[i,value]of this.state.used.entries()){value[0].forEach(key=>this.used[i][0].append(this._create_li(key)));}}
...
return(stateStore.value&&stateStore.value!="")?JSON.parse(stateStore.value):{used:Array(columns).fill().map(()=>[[]]),available:[...Object.keys(proofSteps)]};}
