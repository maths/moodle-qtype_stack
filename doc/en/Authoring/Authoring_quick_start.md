...
A potential response tree is a non-empty acyclic directed graph of _potenital response nodes_.  By default we have one potential response node, and this node is quite simple.
...
From the false branch of Node 1., change the "Next PR" field so it is set to  `[Node 2]`.
...
2. On the false branch set the `mark=1` (well, you may disagree here, but that is up to you!)
