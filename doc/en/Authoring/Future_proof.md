...
For manual testing it is important to provide each input with the correct teacher's answer and that teacher's answer must be in the format so that it would work as raw input.  For example, if you require a floating point number with a specific number of significant figures then do not specify the teacher's answer as a rational number!  Note that you will need to do the same output formatting for the question tests if your question cares about the formatting of the answer.
...
* Maxima has its own `addrow` command.  Please check any questions using `addrow`.
...
Use lexicographical ordering to your advantage, so questions are listed in an order which might be helpful when using them in a quiz.
