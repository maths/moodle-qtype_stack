...
2. `height`: string containing _either_ a positive float + `"px"` (e.g., `"480px"`) or a float in `[0, 100]` + `"%"` (e.g., `"100%"`). Default is `"480px"`. This fixes the height of the window containing the drag-and-drop lists.
3. `width`: string containing _either_ a positive float + `"px"` (e.g., `"480px"`) or a float in `[0, 100]` + `"%"` (e.g., `"100%"`). Default is `"100%"`. This fixes the width of the window containing the drag-and-drop lists.
4. `length`: sring containing a positive integer. Default is `"8"`. This is an alternative way to set the height of the window containing the drag-and-drop lists, and should contain the total number of provided proof steps in the question. An error will occur if one sets values for both `height` and `length` in the block header.
