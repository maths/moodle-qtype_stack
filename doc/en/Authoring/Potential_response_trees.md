...
In each node two expressions are compared using a specified [answer tests](AnswerTests/index.md), and the result is either `true` or `false`. A corresponding branch of the tree has the opportunity to each of the following.
...
The answer note is the concatenation of each answer note from the [answer tests](AnswerTests/index.md) and then the corresponding true/false branch.  This note provides a record of the result of applying each test and the route taken through the tree.
