...
There are other blocks that have roles inside the system and that transfer information required for the operation of the system. If you look at the code these are often called "specialblocks". You can rest sure that there are no realistic use cases for them even in advanced authoring proceses. Practically, none of these ever exist in written out CASText and are only added in as virtual objects during processing. However, if you ever look at the commands going into the CAS you might see these blocks names going in and coming out.
