...
Like [`stackunits`](../stackunits/index.md#stackunits), [`stackbasen`](#stackbasen) "objects" need to be handled with tools.

Unlike [`stackunits`](../stackunits/index.md#stackunits), [`stackbasen`](#stackbasen) "objects" can be nonsense from the start,
...
## sbasen_convert_to_suffix_notation(expression)<a id='sbasen_convert_to_suffix_notation'></a>

Converts all [`stackbasen`](#stackbasen) objects present in an expression to the suffix
...
[`sbasen_trim_padding`](binary/index.md#sbasen_trim_padding).

To avoid issues with different cases will recreate all [`stackbasen`](#stackbasen) objects.
And remap the digits. For digits 10-35 will use ASCII letters and the case
will be defined by the option [`sbasen_construct_default_case`](#sbasen_construct_default_case), by default
...
| Return type | description |
| ----------- | ------------|
| expression | the original expression with all [`stackbasen`](#stackbasen)-objects 	  converted to use "S"-format. |
...
You may use the validation functions [`sbasen_validate`](#sbasen_validate) and
[`sbasen_validate_tree`](#sbasen_validate_tree) to check if the `textform` matches the constraints
set by the `format` and `base`.

You may create these objects directly, but if you wish to create one from
a raw base-10 number you might want to use the [`sbasen_construct_from_base10`](#sbasen_construct_from_base10)
function. Also if you are focused on base-2 then [`sbasen_convert_to_binary`](binary/index.md#sbasen_convert_to_binary)
can be useful for converting from any base or from raw base-10 to base-2.

Conversion back to raw base-10 happens with [`sbasen_convert_to_base10`](#sbasen_convert_to_base10).
