...
* To change the size of the image use the Maxima variable `PLOT_SIZE`.  This is not an argument to plot, instead this is a global variable.  To resize individual plots use a block structure, e.g. `{@(PLOT_SIZE:[250,250],plot(x^2,[x,-1,1]))@}` and note that all following plots will now have width 250.
