...
<code>`<script type="text/x-mathjax-config"> MathJax.Hub.Config({`<br>
`        MMLorHTML: { prefer: "HTML" },tex2jax: {`<br>
`            displayMath: [['$$', '$$'], ['\\[', '\\]']],`<br>
`            inlineMath:  [['$',  '$' ], ['\\(', '\\)']]`<br>
`        }`<br>
`      });`<br>
`</script>`<br>
`<script type="text/javascript" src="http://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS_HTML"></script>`</code>
