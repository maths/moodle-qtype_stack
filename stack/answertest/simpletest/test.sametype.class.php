...
/**
 * Unit tests for STACK_AnsTest_SameType.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class STACK_AnsTest_SameType_test extends UnitTestCase {

    public function test_is_true() {
        $at = new STACK_AnsTest_SameType('x+1', 'x^3+x');
        $this->assertTrue($at->doAnsTest());
        $this->assertEqual(1, $at->getATMark());
    }

    public function test_is_false() {
        $at = new STACK_AnsTest_SameType('x^2+2*x-1', '{(x+1)^2}');
