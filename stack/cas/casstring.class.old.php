...
require_once(__DIR__ . '/casstring.units.class.old.php');

class stack_cas_casstring_old {
    ...
    private function validate($security='s', $syntax=true, $insertstars=0, $allowwords='') {
        ...
        if ($this->units) {
            $stringles = stack_cas_casstring_units_old::make_units_substitutions($stringles);
        }
    ...
    private function check_security($stringles, $security, $rawallowwords) {
        ...
        if (self::$cache === false) {
            self::$cache = array(
                    ...
                    'merged-sallow' => array_merge(self::$studentallow, self::$greekupper,
                            stack_cas_casstring_units_old::get_permitted_units(2)),
        ...
        foreach ($out[0] as $key) {
            ...
            if ($this->units) {
                ...
                list ($fndsynonym, $answernote, $synonymerr) = stack_cas_casstring_units_old::find_units_synonyms($key);
            ...
            if (isset(self::$cache['globalforbid'][strtolower($key)])) {
                ...
                if ($security == 't') {
                    ...
                    if (!isset($allow[$key]) && !isset(self::$cache['merged-sallow'][$key])) {
                        ...
                        } else if ($err = stack_cas_casstring_units_old::check_units_case($key)) {
