...
class stack_cas_castext_external extends stack_cas_castext_block {
    ...
    public function process_content($evaluatedcassession, $conditionstack = null) {
        ...
        $name = md5($full_content).sha1($full_content);
...
abstract class stack_cas_castext_external_handler {
     ...
     * Should there be uncertainty on what files will be generated the handler should generate some markker file so that it can identify when all files have been generated and just give it here, as the primary point of this function is to check for the existence of these files and skip the process stepp if they are present.
     */
    public abstract function get_generated_files();

    /**
     * The contents of the external-block are stored in a randomly named file, should the handlers external processing tools require a specific extension you may ask for it here. Should there be multiple files the extensions for the additional files comes from the file labels.
     */
    ...
    /**
     * Generate the files required by this handler based on the files provided as the $label_map. The map will contain a '__SOURCE_CODE__' key that points to the file with the primary content of the block. Other keys may exists if the block contains them. The processing of the handler should be done in the directory of the '__SOURCE_CODE__'-file or in other temporary place and the generated files should be stored under $dataroot/stack.
     */
