...
class castext2_static_replacer {
    ...
    public function extract(string $in): string {
    	...
    	$collector = function($node) use(&$map) {
    		...
    		if ($node instanceof MP_String && $node->parentnode instanceof MP_List && array_search($node, $node->parentnode->items) > 0 && mb_strlen($node->value) > 10) {
    			...
    			if ($node->parentnode->items[0] instanceof MP_String && (
    				...
    				($node->parentnode->items[0]->value === 'jsxgraph' && array_search($node, $node->parentnode->items) > 1)
    				)) {
