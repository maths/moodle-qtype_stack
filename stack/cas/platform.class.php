...
abstract class stack_platform_base {
    ...
    static public function get_names_and_descs() {
        $f = function($k) {return self::get($k)->get_desc();};
    ...
    public function get_image_from_command($cmd) {
        $args = $this->get_arguments_from_command($cmd);
        // Look for an arg that resides within the data directory - this is likely to be the image
        // file:
        ...
        foreach ($args as $arg) {
            ...
            if(strlen($a) > $n && $this->pathname_equals(substr($a, 0, $n), $dd)) {
                if(!$imageNext && !file_exists($a)) {
                    ...
                    $fallback = array($a, $ai);
                } elseif($imageNext || !file_exists($a) || $fi->file($a) == 'binary') {
                    return array($a, $ai);
                }
            }
            if(strlen($arg) > 0) {
    ...
    public function check_image() {
        ...
        if(!$cbao && !$cbo) {
            ...
            if (!($ofn && is_file($ofn))) {
                ...
                return FALSE;
            } elseif($needsToBeExe && !is_executable($ofn)) {
