...
class stack_choice_input extends stack_input {
    ...
    /*
     * ddltype must be one of 'select' dropdown, 'checkbox' or 'radio' .
     * @var string
     */
    protected $ddltype = '';
    ...
    protected function set_ddltype() {
        $this->ddltype = $this->extraoptions['choicetype'];
    }

    protected function internal_contruct() {
        //Default  ddldisplay depens on the ddltype
        $this->ddldisplay = 'casstring';
        if ($this->get_input_type()=='checkbox' || $this->get_input_type()=='radio'){
    ...
    public function contents_to_maxima($contents) {
        if ($this->get_input_type()=='checkbox'){
            $vals = [];
    ...
    public function render(stack_input_state $state, $fieldname, $readonly, $tavalue) {
        ...
        if ($this->get_input_type()=='dropdown'){
            $select = 0;
            ...
            foreach ($values as $key => $ansid) {
                $inputattributes = [
                    'type' => $this->get_input_type(),    //'radio' or 'checkbox'
                    'value' => $key,
                    'id' => $fieldname.'_'.$key
                ];

                // Metadata for JS users.
                $inputattributes['data-stack-input-type'] = $this->get_input_type();
                ...
                if ($this->get_input_type()=='radio' && '' === $key) {
    ...
    protected function get_input_ddl_value($key) {
        ...
        if ($key !== $this->name) {
            throw new stack_exception('stack_choice_input: could not find a value for key '.$key);
        }
        return false;
    }
