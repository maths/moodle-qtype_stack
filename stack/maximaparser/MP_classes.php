 ...
 */
 ...
 class MP_Node {
  ...
  public function debugPrint($originalcode) {
    ...
    foreach ($this->asAList() as $node) {
        ...
        if ($node instanceof MP_EvaluationFlag) {
          $line .= get_class($node);
        } else {
 ...
 class MP_Operation extends MP_Node {
  ...
  public function replace($node, $with) {
   ...
   $this->children = array(&$this->lhs, &$this->rhs);
  }
 ...
 class MP_Indexing extends MP_Node {
   public $target = null; // This is and identifier of a function call
   public $indices = null; // These are MP_List objects.
 ...
 class MP_EvaluationFlag extends MP_Node {
   ...
   public function replace($node, $with) {
    ...
    $this->children = array(&$this->name, &$this->value);
   }
...
function mergePosition($posA,$posB) {
  ...
  $R = array('start'=>$posA['start'],'end'=>$posA['end']);
