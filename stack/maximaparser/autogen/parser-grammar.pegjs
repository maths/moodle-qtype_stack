...
Comment "comment"
  ...
  for (const item of content) {
    if (item instanceof MPAnnotation) {
  ...
  var n = new MPComment(str, annotations);
  n.position = location();
  return n; }
...
AnnotationDetail
  ...
  return $r;
  ?> **/
  var params = identifiers.map(function (value,index){return value[1];});
...
Identifier "identifier"
  ...
  return $r;
  ?> **/
  var n = new MPIdentifier(char+morechars.join(""));
  n.position = location();
  return n; }
...
Statement
  = __? exp:(ExpOp) flags:EvaluationFlag* {
  /** <?php
...
If
 = 'if' _? cond0:ExpOp _? 'then' _? branch0:ExpOp elifs:(_? 'elseif' _? ExpOp _? 'then' _? ExpOp)+ _? 'else' _? branchn:ExpOp{
 /** <?php
 $conds = array($cond0);
 ...
 return $r;
 ?> **/
 var conds = elifs.map(function (value,index){return value[3];});
 var branches = elifs.map(function (value,index){return value[7];});
 branches = branches.concat(branchn);
 var n = new MPIf([cond0].concat(conds),[branch0].concat(branches));
 n.position = location();
 return n;}
 / 'if' _? cond0:ExpOp _? 'then' _? branch0:ExpOp elifs:(_? 'elseif' _? ExpOp _? 'then' _? ExpOp)+ {
 /** <?php
 $conds = array($cond0);
 $branches = array($branch0);
 foreach ($elifs as $el) {
 ...
 n.position = location();
 return n;}
 / 'if' _? cond0:ExpOp _? 'then' _? branch0:ExpOp _? 'else' _? branch1:ExpOp {
 /** <?php
 $conds = array($cond0);
 ...
 return $r;
 ?> **/
 var n = new MPIf([cond0],[branch0,branch1]);
 n.position = location();
 return n; }
 / 'if' _? cond0:ExpOp _? 'then' _? branch0:ExpOp {
 /** <?php
 $conds = array($cond0);
 $branches = array($branch0);
 $r = new MP_If($conds,$branches);
 ...
 return $r;
 ?> **/
 var n = new MPIf([cond0],[branch0]);
 n.position = location();
 return n; }


Loop
 = conf:LoopBit* 'do' _ body:ExpOp {
 /** <?php
 $r = new MP_Loop($body, $conf);
 $r->position = array('start'=>$this->peg_reportedPos,'end'=>$this->peg_currPos);
 return $r;
 ?> **/
 var n = new MPLoop(body, conf);
 n.position = location();
 return n;
 }
 / conf:LoopBit* 'do' body:Group {
 /** <?php
 $r = new MP_Loop($body, $conf);
 $r->position = array('start'=>$this->peg_reportedPos,'end'=>$this->peg_currPos);
 return $r;
...
LoopBit
 = mode:('for'/'from'/'step'/'next'/'in'/'thru'/'while'/'unless') _ modeparam:ExpOp _? {
 /** <?php
 $r = new MP_LoopBit($mode, $modeparam);
 $r->position = array('start'=>$this->peg_reportedPos,'end'=>$this->peg_currPos);
 return $r;
 ?> **/
 var n = new MPLoopBit(mode, modeparam);
 n.position = location();
 return n;
 }
 / mode:('for'/'from'/'step'/'next'/'in'/'thru'/'while'/'unless') '(' _? modeparam:ExpOp _? ')' _? {
 /** <?php
 $r = new MP_LoopBit($mode, $modeparam);
 $r->position = array('start'=>$this->peg_reportedPos,'end'=>$this->peg_currPos);
 return $r;
...
UnaryOp
  ...
  return opBind($r);
  ?> **/
  let n: MPNode = new MPPostfixOp(op2,new MPPrefixOp(op, trg));
  if (opRBind(op) > opLBind(op2)) {
   n = new MPPrefixOp(op, new MPPostfixOp(op2, trg));
  }
  n.position = location();
  return opBind(n);}
  ...
  n.position = location();
  return n;}
  / trg:( Literal / Group / List / FunctionCall / Indexing / Identifier) _? op:PostfixOp {
  /** <?php
...
Operation
  ...
  return opBind($r);
  ?> **/
  var n = new MPOperation(op,lhs,rhs);
  n.position = location();
  return opBind(n); }
  / UnaryOp

DotOp
  = lhs:Expression _? '.' _? rhs:ExpOp {
  /** <?php
...
FunctionCall
  ...
  return $f;
  ?> **/
  let f = new MPFunctionCall(new MPIdentifier('abs'), [arg]);
  f.position = location();
  return f;
  }
...
USubscriptSuperscript "unicode superscript/subscript character"
 = [\u00b2\u00b3\u00b9\u2070-\u209c\u2c7c\u1d62-\u1d6a]

USpecial "special unicode chars"
 = [\u00b5\u0370-\u0371\u037f\u0391-\u0392\u0393-\u0394\u0395-\u0396\u0397-\u0398\u0399-\u039a\u039b-\u039c\u039d-\u039e\u039f-\u03a0\u03a1\u03a3-\u03a4\u03a5-\u03a6\u03a7-\u03a8\u03a9\u03b1-\u03b2\u03b3-\u03b4\u03b5-\u03b6\u03b7-\u03b8\u03b9-\u03ba\u03bb-\u03bc\u03bd-\u03be\u03bf-\u03c0\u03c1-\u03c2\u03c3-\u03c4\u03c5-\u03c6\u03c7-\u03c8\u03c9\u03db\u03dd\u03df\u03e1\u03f7-\u03f8\u03fa-\u03fb\u1d26-\u1d27\u1d28-\u1d29\u1d2a\u2102\u2107\u210a-\u210b\u210c-\u210d\u210e-\u210f\u2110-\u2111\u2112-\u2113\u2115\u2119-\u211a\u211b-\u211c\u211d\u2124\u2126-\u2127\u2128\u212b-\u212c\u212d\u212f-\u2130\u2131\u2133-\u2134\u2135-\u2136\u2137-\u2138\u213c-\u213d\u213e-\u213f\u2145-\u2146\u2147-\u2148\u2149\u2205\u221e\u29b0\uab65]