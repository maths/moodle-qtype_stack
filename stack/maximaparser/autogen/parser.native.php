...
class MP_Parser {
    ...
    private function peg_f34($trg, $op) {
      $r = new MP_PostfixOp($op, $trg);
      $r->position = array('start'=>$this->peg_reportedPos, 'end'=>$this->peg_currPos);
      return opBind($r);
      }
    private function peg_f35($lhs, $op, $rhs) {
      $r = new MP_Operation($op,$lhs,$rhs);
      $r->position = array('start'=>$this->peg_reportedPos,'end'=>$this->peg_currPos);
      return opBind($r);
      }
    private function peg_f36($lhs, $rhs) {
    ...
    private function peg_f39($fnc, $args, $indices) {
    ...
    private function peg_f40($fnc, $args) {
    ...
    private function peg_f42($trg, $indices) {
      $v = array();
      foreach ($indices as $ind)
       $v[] = $ind[1];
    ...
    private function peg_f45($exp, $tail) {
      $items = array($exp);
      foreach ($tail as $el)
       $items[] = $el[3];
      $r = new MP_Set($items);
    ...
    private function peg_parse_() {
      ...
      $s0 = $this->peg_currPos;
      $s1 = array();
      if (peg_char_class_test($this->peg_c157, $this->input_substr($this->peg_currPos, 1))) {
        $s2 = $this->input_substr($this->peg_currPos, 1);
        $this->peg_currPos++;
      } else {
        $s2 = $this->peg_FAILED;
