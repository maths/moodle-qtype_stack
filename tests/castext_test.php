...
final class castext_test extends qtype_stack_testcase {
    ...
    public function test_validation_error_castext_text(): void {
        ...
        $cs = [];
        foreach ($a as $var) {
            $cs[] = stack_ast_container::make_from_teacher_source($var, '', new stack_cas_security(), []);
        }
    ...
    public function test_broken_block_error2(): void {
        ...
        $this->assertFalse($ct->get_valid());
        $this->assertEquals(
            'Expected "comment", "define", "escape", "if" or identifier but "\/" found.',
            $ct->get_errors()
        );
    }
    ...
    public function test_redefine_variables(): void {
        ...
        $session->instantiate();
        $this->assertEquals(
            'Let \(n\) be defined by \({3}\). Now add one to get \({4}\) and square the result \({16}\).',
            $at1->get_rendered()
        );
    }
    ...
    public function test_plot_small_margin(): void {
        ...
        $at1 = castext2_evaluatable::make_from_source(
            'A small plot: {@plot(p, [x,-2,3], [size,200,100], [margin, 1])@}',
            'test-case'
        );
        $this->assertTrue($at1->get_valid());
        $cs2->add_statement($at1);
    ...
    public function test_forbidden_words(): void {
        ...
        $at1 = castext2_evaluatable::make_from_source(
            'This is system cost {@system("rm /tmp/test")@} to create.',
            'test-case'
        );
    ...
    public function test_disp_decimalplaces_error(): void {
        ...
        $err = ['dispdp requires a real number argument.'];
    ...
    public function test_disp_equiv_natural_domain(): void {
        ...
        $this->assertEquals(
            '\[ {\begin{array}{lll} &\frac{1}{x+1}+\frac{1}{x-1}=0&' .
    ...
    public function test_disp_ode1(): void {
        ...
        $at1 = new stack_cas_keyval(
            "p1:'diff(y,x,2)+2*y = 0;p2:ev('diff(y,x,2),simp)+2*ev('diff(y,x,2,z,3),simp) = 0;",
            null,
            123
        );
        $this->assertTrue($at1->get_valid());
    ...
    public function test_strings_in_castext_escaped(): void {
        ...
        $this->assertEquals(
            '\[{This is a string with escaped " strings....}\]',
            $at2->get_rendered()
        );
    }
    ...
    public function test_strings_in_castext_translated(): void {
        ...
        $this->assertEquals('\({a\,{\text{ and }}\, \text{and}}\)', $at2->get_rendered());
    ...
    public function test_numerical_display_1(): void {
        ...
        $this->assert_content_with_maths_equals(
            'The decimal number \({73}\) is written in base \(2\) as \({1001001}\), in base \(7\) as \({133}\), ' .
                'in scientific notation as \({7.3E+1}\) and in rhetoric as \({\text{seventy-three}}\).',
            $at2->get_rendered()
        );
    }
    ...
    public function test_stackintfmt(): void {
        ...
        $expected = 'Standard: \({1234}\). ' .
                ...
                'Ordinal rethoric: \({\text{one thousand two hundred thirty-fourth}}\). ' .
    ...
    public function test_stack_repeat_blocks(): void {
        $st = '[[repeat id="1"]]' .
            ...
            '[[/repeat]]' .
            '[[repeatbutton title="Add another solution" repeat_ids="1" save_state="state1"/]]';
        $s2 = [];
        $cs2 = new stack_cas_session2($s2, null, 0);
        $at2 = castext2_evaluatable::make_from_source($st, 'test-case');
        $cs2->add_statement($at2);
    ...
    public function test_stack_repeat_blocksbutton_noid(): void {
        $st = '[[repeat id="1"]]' .
            '[[input:ans1]] [[validation:ans1]]</p>' .
            '[[/repeat]]' .
            '[[repeatbutton title="Add another solution" save_state="state1"/]]';
        $s2 = [];
        $cs2 = new stack_cas_session2($s2, null, 0);
        $at2 = castext2_evaluatable::make_from_source($st, 'test-case');
        $cs2->add_statement($at2);
        $this->assertFalse($at2->get_valid());
        $this->assertEquals('Repeatbutton block requires a non-empty repeat_ids parameter.', $at2->get_errors(true));
    }
    ...
    public function test_stack_repeat_blocksbutton_notitle(): void {
        $st = '[[repeat id="1"]]' .
            '[[input:ans1]] [[validation:ans1]]</p>' .
            '[[/repeat]]' .
            '[[repeatbutton title=""  repeat_ids="1" save_state="state1"/]]';
        $s2 = [];
        $cs2 = new stack_cas_session2($s2, null, 0);
        $at2 = castext2_evaluatable::make_from_source($st, 'test-case');
        $cs2->add_statement($at2);
        $this->assertFalse($at2->get_valid());
        $this->assertEquals('Repeatbutton block requires a non-empty title parameter.', $at2->get_errors(true));
    }
     ...
     * @covers \qtype_stack\stack_cas_castext2_repeatbutton
     */
    public function test_stack_repeat_blocksbutton_nosavestate(): void {
        $st = '[[repeat id="1"]]' .
            '[[input:ans1]] [[validation:ans1]]</p>' .
            '[[/repeat]]' .
        ...
        $cs2->add_statement($at2);
        $this->assertFalse($at2->get_valid());
        $this->assertEquals('Repeatbutton block requires a non-empty save_state parameter.', $at2->get_errors(true));
    }
    ...
    public function test_stack_var_makelist(): void {
        ...
        $this->assertEquals(
            '\({\left[ k_{0} , k_{1} , k_{2} , k_{3} , k_{4} , k_{5} \right]}\) ' .
    ...
    public function test_stack_simp_false_castext(): void {
        ...
        $this->assertEquals(
            'Confirm simplification is on: \({2}\).  ' .
    ...
    public function test_display_polarform(): void {
        ...
        $this->assertEquals(
            '\[{\sqrt{2}\cdot e^{\frac{\mathrm{i}\cdot \pi}{4}}}\]',
            $at2->get_rendered()
        );
    }
    ...
    public function test_display_logic(): void {
        ...
        $cs2->instantiate();
        $this->assertEquals(
            '\({A\,{\text{ and }}\, B}\), \({A\,{\text{ and }}\, B}\). ' .
                '\({A\,{\text{ or }}\, B}\), \({A\,{\text{ or }}\, B}\). ' .
                '\({\left(A\,{\text{ or }}\, B\right)\,{\text{ and }}\, C}\); ' .
                '\({A\,{\text{ and }}\, B\,{\text{ or }}\, C}\). ' .
                '\({\left(A\,{\text{ or }}\, B\right)\,{\text{ and }}\, C}\); ' .
                '\({A\,{\text{ and }}\, B\,{\text{ or }}\, C}\). ' .
                '\({{\rm not}\left( A \right)}\). ' .
                '\({A\,{\text{ nand }}\, B\,{\text{ and }}\, C\,{\text{ nor }}\, ' .
                'D\,{\text{ xor }}\, E\,{\text{ or }}\, F\,{\text{ implies }}\, G\,{\text{ xnor }}\, H}\).',
            $at2->get_rendered()
        );
        ...
        $cs2->instantiate();
        $this->assertEquals(
            '\({A\land B}\), \({A\land B}\). \({A\lor B}\), \({A\lor B}\). ' .
    ...
    public function test_display_tables(): void {
        ...
        $cs2->instantiate();
        $this->assertEquals(
            '\({\begin{array}{c|c} x & x^3\\\\ \hline -1 & -1 \\\\ ' .
        ...
        $cs2->instantiate();
        $this->assertEquals(
            '\({\begin{array}{c|c|c} a & b & a\,{\text{ implies }}\, b\\\\ ' .
        ...
        $cs2 = $at1->get_session();
        $at2 = castext2_evaluatable::make_from_source(
            '{@truth_table(a xnor b)@}',
            'test-case'
        );
        $this->assertTrue($at2->get_valid());
        $cs2->add_statement($at2);
        $cs2->instantiate();
        $this->assertEquals('\({\begin{array}{c|c|c} a & b & a\,{\text{ xnor }}\, b\\\\ \hline \mathbf{False} ' .
        ...
        $cs2 = $at1->get_session();
        $at2 = castext2_evaluatable::make_from_source(
            '{@table_difference(truth_table(a xor b), truth_table(a implies b))@}',
            'test-case'
        );
        $this->assertTrue($at2->get_valid());
        $cs2->add_statement($at2);
        $cs2->instantiate();
        $this->assertEquals(
            '\({\begin{array}{c|c|c} a & b & \color{red}{\underline{a\,{\text{ xor }}\, b}}\\\\ ' .
    ...
    public function test_tex_aligned(): void {
        ...
        $cs2 = $at1->get_session();
        $at2 = castext2_evaluatable::make_from_source(
            '{@lrparens(".", aligned([x^2+2,stackeq(3)],[x^3,stackeq(4)]), "\\\\}")@}',
            'test-case'
        );
        $this->assertTrue($at2->get_valid());
        $cs2->add_statement($at2);
        ...
        $this->assertEquals(
            '\({\left.\begin{aligned}x^2+2&=3\cr x^3&=4\cr \end{aligned}\right\}}\)',
            $at2->get_rendered()
        );
    }
    ...
    public function test_display_tree(): void {
        ...
        $cs2->add_statement($at2);
        $cs2->instantiate();
        $this->assertEquals("\({x=a\,{\\text{ or }}\, b}\): <ul class='algebratree'><li><span class='op'>\(\,{\\text{ or }}\, \)" .
            ...
            "\({x=\left(a\,{\\text{ or }}\, b\\right)}\): <ul class='algebratree'><li><code>=</code><ul><li><span class='atom'>" .
            "\(x\)</span></li><li><span class='op'>\(\,{\\text{ or }}\, \)</span><ul><li><span class='atom'>\(a\)</span>" .
    ...
    public function test_display_tree_texput(): void {
        ...
        $this->assertEquals(
            "<ul class='algebratree'><li><span class='op'>\(\diamond\)</span><ul><li>" .
    ...
    public function test_display_tree_strings(): void {
        ...
        $this->assertEquals(
            "<ul class='algebratree'><li><span class='op'>\(\,{\\text{ and }}\, \)</span><ul>" .
    ...
    public function test_include_local(): void {
        ...
        $this->assertEquals(
            'Underscore characters are not permitted in this input.',
            $at2->get_rendered()
        );
    }
    ...
    public function test_unary_minus_zeros(): void {
        ...
        $this->assertEquals(
            '\({x-0}\), \({x-0.0}\), \({x\cdot \left(-0.0\right)}\), \({-27}\).',
            $at1->get_rendered()
        );
    }
