...
final class input_algebraic_test extends qtype_stack_testcase {
    ...
    public function test_render_zero(): void {
        ...
        $this->assertEquals(
            ...
            . 'data-stack-input-list-separator="," />',
            $el->render(
                new stack_input_state(stack_input::VALID, ['0'], '', '', '', '', ''),
                'stack1__ans1',
                false,
                null
            )
    ...
    public function test_render_pre_filled(): void {
        ...
        $this->assertEquals(
            '<input type="text" name="stack1__test" id="stack1__test" size="16.5" '
            . 'style="width: 13.6em" autocapitalize="none" spellcheck="false" class="algebraic" value="x+y" '
            . 'data-stack-input-type="algebraic" data-stack-input-decimal-separator="." '
            . 'data-stack-input-list-separator="," />',
            $el->render(
                new stack_input_state(stack_input::VALID, ['x+y'], '', '', '', '', ''),
                'stack1__test',
                false,
                null
            )
    ...
    public function test_render_disabled(): void {
        ...
        $this->assertEquals(
            ...
            . 'data-stack-input-list-separator="," />',
            $el->render(
                new stack_input_state(stack_input::VALID, ['x+1'], '', '', '', '', ''),
                'stack1__input',
                true,
                null
            )
    ...
    public function test_syntaxhint(): void {
        ...
        $this->assertEquals(
            '<input type="text" name="stack1__sans1" id="stack1__sans1" size="16.5" '
            . 'style="width: 13.6em" autocapitalize="none" spellcheck="false" class="algebraic" value="[?, ?, ?]" '
            . 'data-stack-input-type="algebraic" data-stack-input-decimal-separator="." '
            . 'data-stack-input-list-separator="," />',
            $el->render(
        ...
        $this->assertEquals(
            '\[ \left[ \color{red}{?} , \color{red}{?} , \color{red}{?} \right] \]',
    ...
    public function test_validate_student_response_algebraic_11(): void {
        ...
        $this->assertEquals(
            '\[ \mathrm{i}\cdot \left(x+1\right)+j\left(2\cdot x+3\right)+k\left(3\cdot x+4\right) \]',
            $state->contentsdisplayed
        );
        ...
        $this->assertEquals('i*(x+1)+j*(2*x+3)+k*(3*x+4)', $state->contentsmodified);
        $this->assertEquals(
            '\[ \mathrm{i}\cdot \left(x+1\right)+j\cdot \left(2\cdot x+3\right)+' .
        ...
        $this->assertEquals(
            '\[ j\cdot \left(2\cdot x+3\right)+k\cdot \left(3\cdot {\it ab}+4\right)+\sin \left( x \right) \]',
            $state->contentsdisplayed
        );
        ...
        $this->assertEquals(
            '\[ j\cdot \left(2\cdot x+3\right)+k\cdot \left(3\cdot a\cdot b+4\right)+\sin \left( x \right) \]',
            $state->contentsdisplayed
        );
    ...
    public function test_validate_student_response_ex(): void {
        ...
        $this->assertEquals(
            '\[ 3\cdot {\it ex}+2\cdot {\it ey}+5\cdot {\it ez} \]',
            $state->contentsdisplayed
        );
    ...
    public function test_validate_student_response_subscripts(): void {
        ...
        $state = $el->validate_student_response(
            ['sans1' => 'rho*z*V/(4*pi*epsilon[0]*(R^2+z^2)^(3/2))'],
            $options,
            'x^2+1/3',
            new stack_cas_security(false, '', '', ['tans'])
        );
    ...
    public function test_validate_student_response_sametype_subscripts_true_invalid(): void {
        ...
        $this->assertEquals('{mu_0*(I_1-I_2)}', $state->contentsmodified);
        $this->assertEquals(
            '\[ \left \{{\mu}_{0}\cdot \left({I}_{1}-{I}_{2}\right) \right \} \]',
            $state->contentsdisplayed
        );
        if ($this->adapt_to_new_maxima('5.34.2')) {
            // Why change the order here?
    ...
    public function test_validate_student_response_display_noundiff(): void {
        ...
        $el->set_parameter('sameType', false);
        $state = $el->validate_student_response(
            ['sans1' => 'noundiff(y/x^2,x,1)-(2*y)/x = x^3*sin(3*x)'],
            $options,
            'diff(y/x^2,x,1)-(2*y)/x = x^3*sin(3*x)',
            new stack_cas_security()
        );
        $this->assertEquals(stack_input::VALID, $state->status);
    ...
    public function test_validate_student_response_extra_evaluation(): void {
        ...
        $this->assertEquals(
            "Maxima's extra evaluation operator <code>''</code> is not supported by STACK.",
            $state->errors
        );
        $this->assertEquals("''%_E(%_E(noundiff(y/x^2,x,1)))", $state->contentsmodified);
        $this->assertEquals(
            "<span class=\"stacksyntaxexample\">''diff(y/x^2,x,1)</span>",
            $state->contentsdisplayed
        );
    }
    ...
    public function test_validate_student_response_allowwords_false(): void {
        ...
        $el = stack_input_factory::make('algebraic', 'sans1', '2*x');
        $state = $el->validate_student_response(
            ['sans1' => 'unknownfunction(x^2+1)+3*x'],
            $options,
            '2*x',
            new stack_cas_security(false, '', '', ['ta'])
        );
    ...
    public function test_validate_student_response_single_variable(): void {
        ...
        $this->assertEquals(
            '\[ \frac{\cos \left( a\cdot x \right)}{x\cdot \ln \left( x \right)} \]',
            $state->contentsdisplayed
        );
    }
    ...
    public function test_validate_student_response_simp_1(): void {
        ...
        $this->assertEquals(
            '\[ \left[ 1 , 4 , 9 , 16 , 25 , 36 , 49 , 64 \right] \]',
            $state->contentsdisplayed
        );
    }
    ...
    public function test_validate_student_response_star_space_3(): void {
        ...
        $this->assertEquals('missing_stars | spaces', $state->note);
        $this->assertEquals(
            'You seem to be missing * characters. ' .
    ...
    public function test_validate_student_response_almost_cardano(): void {
        ...
        $state = $el->validate_student_response(
            ['sans1' => 'x=(-q+-sqrt(q^2-p^3))^(1/3)+(-q+-sqrt(q^2-p^3))^(1/3)'],
            $options,
            'x=-b#pm#sqrt(b*c^2-a)',
            new stack_cas_security(false, '', '', ['tans'])
        );
    ...
    public function test_validate_student_response_subtle_pm(): void {
        ...
        $this->assertEquals(
            '\[ a\cdot x+a\cdot y+\left(-b\right)\cdot x+\left(-b\right)\cdot y \]',
            $state->contentsdisplayed
        );
        $this->assertEquals('', $state->note);
    }
    ...
    public function test_validate_student_response_realsets_sametype_err(): void {
        ...
        $this->assertEquals('', $state->note);
        $this->assertEquals(
            'Interval construction must have exactly two arguments, so this must be an error: ' .
        ...
        $this->assertEquals('', $state->note);
        $this->assertEquals(
            'When constructing a real interval the end points must be ordered. ' .
        ...
        $this->assertEquals(
            'Your answer should be a subset of the real numbers. ' .
    ...
    public function test_validate_student_response_no_dot_dot(): void {
        ...
        $this->assertEquals(
            '<span class="stacksyntaxexample">3.14.2.78</span>',
            $state->contentsdisplayed
        );
    ...
    public function test_missing_validator_(): void {
        ...
        $this->assertEquals('\[ x^2 \]', $state->contentsdisplayed);
        $this->assertEquals(
            'The optional validator threw internal Maxima errors.',
            $state->errors
        );
    }
    ...
    public function test_decimal_output_0(): void {
        ...
        $this->assertEquals(
            'You have used the full stop <code>.</code>, but you must use the comma <code>,</code> as a ' .
            'decimal separator!',
            $state->errors
        );
        $this->assertEquals(
            '<input type="text" name="state" id="state" size="16.5" style="width: 13.6em" '
    ...
    public function test_decimal_output_matrix_2(): void {
        ...
        $state = $el->validate_student_response(
            ['state' => 'matrix([3,1415;2,71]).matrix([1];[2])'],
            $options,
            'matrix([3.1415,2.71])',
            new stack_cas_security()
        );
        ...
        $this->assertEquals('', $state->contentsmodified);
        $this->assertEquals(
            '<span class="stacksyntaxexample">matrix([3,1415;2,71]).matrix([1];[2])</span>',
            $state->contentsdisplayed
        );
    ...
    public function test_validate_invalid_final_chars(): void {
        ...
        $this->assertEquals(
            '\'=\' is an invalid final character in <span class="stacksyntaxexample">y=</span>',
            $state->errors
        );
        $this->assertEquals('', $state->contentsmodified);
        ...
        $this->assertEquals('finalChar', $state->note);
        $this->assertEquals(
            '\'=\' is an invalid final character in ' .
    ...
    public function test_validate_invalid_chained_op(): void {
        ...
        $this->assertEquals(
            'You appear to have "chained operators" e.g. ' .
